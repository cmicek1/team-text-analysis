...
dict_file = "cata-dict.xlsx"
json_file_path = "task_cutoffs.json"
...
def analyze_text(transcript_path, output_speaker_dir, result_path, task_cutoff):
    ...
    speakers = speaker_time_series_df[speaker_time_series_df['speaker'] != 'None']['speaker'].unique()
    ...
    group_sum_time_series_df = speaker_time_series_df.groupby(['window_start', 'window_end']).agg(
        {**{col: 'sum' for col in speaker_time_series_df.columns if
            col not in ['window_start', 'window_end', 'speaker']},
         'speaker': speaker_agg}
    ).reset_index()
    ...
    mean_agg = lambda x: x.sum()/len(speakers)

    group_mean_time_series_df = speaker_time_series_df.groupby(['window_start', 'window_end']).agg(
        {**{col: mean_agg for col in speaker_time_series_df.columns if
            col not in ['window_start', 'window_end', 'speaker']},
         'speaker': speaker_agg}
    ).reset_index()
    ...
    group_sum_time_series_df.to_csv(result_path.format(suffix=sum_suffix), index=False)
    print(f"Analyzing {transcript_path} -> Saving results to {result_path.format(suffix=sum_suffix)}")

    group_mean_time_series_df.to_csv(result_path.format(suffix=mean_suffix), index=False)
    print(f"Analyzing {transcript_path} -> Saving results to {result_path.format(suffix=mean_suffix)}")
...
def main():
    ...
    for i in range(1, num_groups + 1):
        ...
        if transcript_files:
            ...
            output_path = os.path.join(output_group_dir, f"{prefix}_group_text_analysis" + "{suffix}.csv")
